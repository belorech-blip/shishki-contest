...
try {
    ...
    if ($adminsCount === 0) {
        ...
        $stmt->execute([':login' => $login, ':password_hash' => $hash, ':token' => $token, ':name' => 'Администратор']);
        json_response(['success' => true, 'message' => 'Первый администратор создан. Вход выполнен.', 'token' => $token, 'admin' => ['id' => (int)$pdo->lastInsertId(), 'login' => $login, 'name' => 'Администратор', 'role' => 'admin']]);
    }
    ...
    $update->execute([':token' => $token, ':id' => (int)$admin['id']]);

    json_response(['success' => true, 'message' => 'Вход выполнен', 'token' => $token, 'admin' => ['id' => (int)$admin['id'], 'login' => $admin['login'], 'name' => $admin['name'] ?: $admin['login'], 'role' => $admin['role']]]);
